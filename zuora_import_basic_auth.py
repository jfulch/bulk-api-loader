...
def read_data_from_csv_content(csv_content):
    data = []
...
def get_new_token(retries=3, delay=5):
    ...
    for attempt in range(retries):
        try:
            response = requests.post(ZUORA_AUTH_URL, auth=HTTPBasicAuth(ZUORA_USER_NAME, ZUORA_PASSWORD), headers={
...
if __name__ == "__main__":
    ...
    # Read CSV content from standard input
    csv_content = json.loads(sys.stdin.read())
    data = read_data_from_csv_content(csv_content)
